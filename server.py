...
def get_cv2_image_from_base64_string(b64str):
    ...
    :param b64str: Base64 string of the image
    :return: cv2 image if successful, None otherwise
    '''
    try:
        logging.info("Received base64 string: %s", b64str[:30])  # Log the beginning of the base64 string
        if ',' in b64str:
        ...
        # Check if nparr is empty or not
        if nparr.size == 0:
...
def classify_image_endpoint():
    try:
        ...
        logging.info("Received image data: %s", image_data[:30])  # Log the beginning of the image data
        if not image_data:
